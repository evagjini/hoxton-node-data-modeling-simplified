...
app.post("/interviews", (req, res) => {
  ...
  if (typeof req.body.successful !== "string") {
    errors.push("  Successful It is not a number!");
  }
...
app.post("/companies", (req, res) => {
        ...
        res.send(createNewCompany.run({ name, city }));
...
app.post("/employees", (req, res) => {
  ...
  res.send(createNewEmployee.run({ name, email, position, companyId }));
});
...
app.delete("/companies/:id", (req, res) => {
  ...
  if (employee) {
    res.send(fireSomeone.run({ id: employee.id }));
  } else {
